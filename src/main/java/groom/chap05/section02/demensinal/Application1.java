package groom.chap05.section02.demensinal;
...
//        iarr1 = new int[][4];                   //주소를 묶어서 관리할 배열의 크기를 지정하지 않으면 에러가 발생함
        ...
        /* 참고 : 위에서 진행한 2,3 번은 동시에 진행할 수 도 있다.
         ...
         * 서로 같은 길이의 여러 개 배열을 하나로 묶어 관리하는 2차원 배열울 정변배열 이라고 하며,
         * 서로 길이가 같이 않은 여러 개의 배열을 하나로 묶어 관리하는 2차원 배열을 가변배열 이라고 한다.
         */
        iarr2 = new int[3][5];

        /* 4. 각 배열의 인덱스에 차례로 접근해서 값 출력 */
        /* 할당 후에 아무 값을 대입하지 않아도 heap 영역에는 값이 없는 상태로 공간을 생성할 수 없기 때문에 기본값이 들어있다. */
        ...
        /* 2번 인덱스의 배열 값 출력 */
        for(int i = 0 ; i < iarr2[2].length; i++){
            System.out.println(iarr2[2][i] + " ");
        }
        System.out.println();

        /* 방금 출력한 내용에서 규칙을 찾아보자.
         *  배열의 처음과 끝을 차례로 접근하여 출력하는 구문 자체를 반복하고 있다.
         *  따라서 한 개 인덱스가 가진 배열의 값을 출력하는 반복문을 3번 반복하는 반복문을 작성해 보자
         */

        /* 4-2. 중첩 for문을 이용해서 배열의 값 출력 */
        System.out.println("중첩 for문을 이용해서 배열의 값 출력");
