package groom.chap05.section03.copy;
...
public class Application2 {
    ...
    public static void main(String[] args) {
         ...
         * 이 중 가장 높은 성능을 보이는 것은 순수 배열의 복사를 위해 만들어진 arrycopy() 메소드이며
         * 가장 많이 사용되는 방식은 좀 더 유연한 방식인 copyOf() 메소드 이다.
         * clone()은 이전 베열과 같은 배열밖에 만들 수 없다는 특징을 가지고
        ...
        /* 배열을 출력 기능을 하는 메소드를 따로 만들어서 호출한다. */
        print(originArr);
        ...
        /* 원본배열, 복사를 시작할 인덱스, 복사본 배열, 복사본 배열에서 복사가 시작될 위치, 복사할 길이 의미를 가진다. */
        System.arraycopy(originArr, 0, copyArr3, 3, originArr.length);

        print(copyArr3);                        // 복사한 만큼의 값은 같지만 길이도 다르고 주소도 다르다.

        /* 4. Arrays의 copyOf()를 이용한 복사 */
        /* 시작인덱스부터 원하는 길이만큼 복사해서 사용 가능하다. */
        int[] copyArr4 = Arrays.copyOf(originArr, 7);

        print(copyArr4);                        // 복사한만큼의 값은 값지만 길이도 다르고 주소도 다르다.
    }
